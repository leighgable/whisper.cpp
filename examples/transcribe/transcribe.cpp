...
struct whisper_params {
    ...
    std::string model     = "models/ggml-tiny.en-q5_1.bin";
...
int audio_init_ma(my_app_state& app, whisper_params& params) {
    ...
    if (params.save_audio) {
        ...
        if (ma_encoder_init_file(("audio_" + std::to_string(UTC) + ".wav").c_str(), &aEncoderConfig, &app.aEncoder.value()) != MA_SUCCESS) {
            fprintf(stderr, "Failed to create output audio file: audio_%ld.wav\n", UTC);
            return -1;
        }
    }
    ...
    if (result != MA_SUCCESS) {
        ...
        fprintf(stdout, "Audio capture device %s initialized.\n", app.device.capture.name);
    }
...
int main(int argc, char ** argv) {
    ...
    while (is_running) {
        ...
        if (!use_vad) {
            ...
            for(int num_frames = 0; num_frames < 2000; num_frames++) {
                float v;
                app.ringbuffer.wait_dequeue(v);
                pcmf32_new[num_frames] = v;
            }
            ...
            if (::vad_simple(pcmf32_new, WHISPER_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, false)) {
                ...
                for(int num_frames = 0; num_frames < params.length_ms; num_frames++) {
                    float v;
                    app.ringbuffer.wait_dequeue(v);
                    pcmf32_new[num_frames] = v;
                }
